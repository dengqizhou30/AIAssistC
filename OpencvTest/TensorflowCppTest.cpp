...
void Mat_to_Tensor_UINT(Mat img, Tensor* output_tensor, int input_rows, int input_cols)
{
    //图像进行resize处理
    resize(img, img, cv::Size(input_cols, input_rows));
    //imshow("resized image",img);
    ...
    cv::Mat tempMat(input_rows, input_cols, CV_8UC3, p);   //注意转换的图像为彩色图还是灰度图
    img.convertTo(tempMat, CV_8UC3, 255);
}
...
int main_bak2()
{
   ...
   //const string ModelFile = "../../Data/model/mobilenet/ssd_mobilenet_v3.pb";
    ...
    //const string ModelFile = "../../Data/model/tensorflow/efficientdet_d0/saved_model.pb";
    ...
    Tensor input_tensor1(DT_UINT8, TensorShape({ 1,input_height,input_width,3 }));
    //将Opencv的Mat格式的图片存入tensor
    Mat_to_Tensor_UINT(mat2, &input_tensor1, input_height, input_width);
    ...
    string input_tensor_name = "image_arrays";
    string output_node = "detections";
