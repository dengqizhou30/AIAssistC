...
int main_bak3()
{
   ...
   */
   ...
   //const string ModelFile = "../../Data/model/mobilenet/ssd_mobilenet_v3.pb";
    ...
    const string ModelFile = "../../Data/model/tensorflow/efficientdet_d0/";
    //const string ModelFile = "../../Data/model/tensorflow/efficientdet_v2/";
    ...
    auto out = model({ {"serving_default_input_tensor:0", input} }, { "StatefulPartitionedCall:0", "StatefulPartitionedCall:1", "StatefulPartitionedCall:2", "StatefulPartitionedCall:4" });
    ...
    for (int count = 0; count < 100; count++) {
        ...
        */
        ...
        confidences = out[3].get_data<float_t>();
        boxes_float = out[1].get_data<float_t>();
        boxes = vector<Rect>();
        for (int i = 0; i < boxes_float.size(); i += 4) {
            //object_detection坐标格式为[ymin , xmin , ymax , xmax] 
            Rect rect;
            rect.y = boxes_float[i] * input_width;
            rect.x = boxes_float[i + 1] * input_height;
            rect.height = boxes_float[i + 2] * input_width - rect.y;
            rect.width = boxes_float[i + 3] * input_height - rect.x;
